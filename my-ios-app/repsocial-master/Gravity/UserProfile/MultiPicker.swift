...
public struct MultiPicker<Label: View, SelectionValue: Hashable & CustomStringConvertible, Content: View>: View {
    ...
    @Environment(\.maxValues) var maxValues
    
    var selectionEnabled: Bool {
        guard case let .multiple(binding) = selection,
              let maxValues else { return true }
        return binding.wrappedValue.count < maxValues
    }
                ...
                MultiPickerSelectionList<SelectionValue, Content>(selection: selection, indicatorPosition: selectionIndicatorPosition, content: content)
                    ...
                    .toolbar {
                        ToolbarItem(placement: .topBarTrailing) {
                            if case let .multiple(binding) = selection,
                                let maxValues {
                                Text("\(binding.wrappedValue.count) of \(maxValues)")
                            } else {
                                EmptyView()
                            }
                        }
...
fileprivate enum SelectionBinding<SelectionValue: Hashable>: Equatable {
    ...
    func select(_ bound: SelectionValue, selectionEnabled: Bool) {
        switch self {
        case let .single(binding):
...
fileprivate struct MultiPickerSelectionList<SelectionValue: Hashable, Content: View>: View {
    ...
    @Environment(\.selectionEnabled) var selectionEnabled
        ...
        List {
            content.childViews { children in
                ForEach(children) { child in
                    ...
                    .onTapGesture {
                        tag.map { selection.select($0, selectionEnabled: selectionEnabled) }
                    }
                }
            }
        }
    ...
    fileprivate init(
        ...
        indicatorPosition: SelectionIndicatorPosition = .trailing,
    ...
    fileprivate init(
        selection: SelectionBinding<SelectionValue>,
        indicatorPosition: SelectionIndicatorPosition = .trailing,
        @ViewBuilder content: () -> Content
    ) {
